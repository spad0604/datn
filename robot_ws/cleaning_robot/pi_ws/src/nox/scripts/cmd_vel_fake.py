...
class CmdVelTimeout:
    def __init__(self):
        ...
        self.pub_serial = rospy.Publisher('serial_ard_tx', String, queue_size=1)
    ...
    def cmd_vel_callback(self, msg):
        ...
        self.count_delay+=1
        if self.count_delay>=30:
            if msg.linear.x <0:
                angular_z=-angular_z
            if angular_z > self.angular_threshold:
                self.pub_serial.publish("R_ON")
                rospy.loginfo_throttle(2, "Bật đèn TRÁI (L_ON)")
            elif angular_z < -self.angular_threshold:
                self.pub_serial.publish("L_ON")
                rospy.loginfo_throttle(2, "Bật đèn PHẢI (R_ON)")
            self.count_delay=0
