"""WebSocket / STOMP client for robot <-> BE communication.

This module provides a clean API for connecting robot to BE server via STOMP protocol.
"""
...
class ServerService:
    """Backward-compatible API backed by BE websocket + REST.

    This class keeps the old public API so other scripts don't need
    large changes when we move the implementation here.
    """
