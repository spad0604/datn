...
class DualSerialNode:
    def __init__(self):
        ...
        try:
            ...
            rospy.logwarn("Không mở được cổng serial Arduino (%s): %s", port1, e)
            self.ser_ard = None
        if self.use_sim:
    ...
    def send_to_port1(self, msg):
        if self.ser_ard is not None and self.ser_ard.is_open:
            ...
            rospy.logwarn_throttle(10, "Arduino serial is not available; drop serial_ard_tx: %s", msg.data)

    def send_to_port2(self, msg):
        if self.ser_sim is not None and self.ser_sim.is_open:
